    ...
    @php
        function getPaymentBadge($status) {
            $map = [
                ...
                'Paid' => 'badge-paid',
                'Waived' => 'badge-waived',
                'Verification Pending' => 'badge-processing', // Blue/Indigo
            ];
            ...
            <div class="table-filters">
                <form action="{{ route('secretary.document-services') }}" method="GET">
                    ...
                    <select name="status" class="filter-dropdown" onchange="this.form.submit()">
                        ...
                        <option value="Verification Pending" {{ request('status') === 'Verification Pending' ? 'selected' : '' }}>Verification Pending (Payment)</option>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {{-- UPDATED: Show Payment Method --}}
                            @if($request->payment_method)
                                <div style="font-size: 0.75rem; color: #6B7280; margin-top: 4px;">
                                    @if($request->payment_method === 'Online')
                                        <i class="fas fa-mobile-alt text-primary"></i> Online
                                    @elseif($request->payment_method === 'Cash')
                                        <i class="fas fa-coins text-success"></i> Cash
                                    @else
                        ...
                        <td class="table-actions">
                            {{-- View/Download Icon for Completed --}}
                            @if($request->status === 'Completed' && $request->generated_file_path)
                                <a href="{{ Storage::url($request->generated_file_path) }}" target="_blank" class="text-success" title="View Document" style="font-size: 1.1rem; margin-right: 8px;">
    ...
    <div class="card-grid">
        ...
        <div class="doc-type-card">
            ...
            <div class="doc-type-card-body">
                ...
                {{-- UPDATED: Show actual dynamic fields --}}
                @if(!empty($type->custom_fields))
                    <h4>Required Information</h4>
                    <ul>
                        @foreach($type->custom_fields as $field)
                            <li>
                ...
                @else
                    <h4>Requirements</h4>
                    <ul>
                        <li><i class="fas fa-info-circle"></i> Standard Requirements</li>
                    </ul>
                @endif
            </div>
