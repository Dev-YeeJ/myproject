...
<div id="newProjectModal" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <form action="{{ route('captain.project.store') }}" method="POST">
            @csrf
        ...
        <form id="updateForm" action="" method="POST">
            ...
            <div class="form-group">
                ...
                <small style="color: #6B7280; display:block; margin-top:5px;">
                    <i class="fas fa-lock"></i> To add expenses, use the "Add Expense" button on the card.
                </small>
            </div>
...
<div id="addExpenseModal" class="modal" style="display:none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3 class="modal-title"><i class="fas fa-coins" style="color: #FFA500;"></i> Record Expense</h3>
        </div>
        <p style="color:#666; font-size:0.9rem; margin-bottom:15px; margin-top: -10px;">This will automatically update the Financial Management records.</p>
        
        <form action="{{ route('captain.transaction.store') }}" method="POST">
            @csrf
            <input type="hidden" name="type" value="expense">
            <input type="hidden" name="project_id" id="exp_project_id">
            
            <div class="form-group">
                <label class="form-label">Project</label>
                <input type="text" id="exp_project_name" readonly class="form-control" style="background:#F3F4F6;">
            </div>

            <div class="form-group">
                <label class="form-label">Description</label>
                <input type="text" name="title" placeholder="e.g. Cement Bags" class="form-control" required>
            </div>

            <div class="form-group">
                <label class="form-label">Category</label>
                <input type="text" name="category" id="exp_category" readonly class="form-control" style="background:#F3F4F6;">
            </div>
            ...
            <div style="text-align:right; margin-top: 20px;">
                <button type="button" onclick="closeModal('addExpenseModal')" class="btn-secondary" style="margin-right:10px;">Cancel</button>
                <button type="submit" class="btn-primary" style="background: #FFA500;">Save Expense</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function addExpense(project) {
        document.getElementById('exp_project_id').value = project.id;
        document.getElementById('exp_project_name').value = project.title;
        document.getElementById('exp_category').value = project.category; 
